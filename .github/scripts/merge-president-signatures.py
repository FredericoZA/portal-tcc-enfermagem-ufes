...
# Sidebar: remove redundant Assinaturas entry.
sidebar_path = Path('src/components/Sidebar.tsx')
sidebar = sidebar_path.read_text(encoding='utf-8')
sidebar = sidebar.replace('  ShieldCheck\n', '')
sidebar = re.sub(r"\n\s*assinaturas:\s*\{ id: 'assinaturas'.*?\},", '', sidebar, count=1)
...
sidebar = re.sub(
    r"\n\s*if \(!order\.includes\('assinaturas'\)\) \{\n\s*const settingsIndex = order\.indexOf\('configuracoes'\);\n\s*order\.splice\(settingsIndex >= 0 \? settingsIndex : order\.length, 0, 'assinaturas'\);\n\s*\}",
    '',
    sidebar,
    count=1,
)
if "id: 'assinaturas'" in sidebar or "order.includes('assinaturas')" in sidebar:
    raise SystemExit('Assinaturas still present in sidebar navigation.')
sidebar_path.write_text(sidebar, encoding='utf-8')

# App: keep old tab as backwards-compatible alias, but render President area.
app_path = Path('src/App.tsx')
app = app_path.read_text(encoding='utf-8')
app = app.replace("const SignatureQueuePage = lazy(() => import('./pages/SignatureQueuePage').then((module) => ({ default: module.SignatureQueuePage })));\n", '')
app = replace_once(
    app,
    "      case 'coordenador':\n        return <CoordenadorPage onSelectProcess={(id) => handleSelectProcess(id, false)} />;\n      case 'assinaturas':\n        return <SignatureQueuePage />;",
    "      case 'coordenador':\n      case 'assinaturas':\n        return <CoordenadorPage onSelectProcess={(id) => handleSelectProcess(id, false)} />;",
    'App signature route alias',
)
app = replace_once(
    app,
...
# President page: integrate Asten actions, bulk signing, status and standard filter chips.
page_path = Path('src/pages/CoordenadorPage.tsx')
page = page_path.read_text(encoding='utf-8')
page = page.replace("import { ProcessData } from '../types';", "import { ProcessData, SignatureJob, AstenIntegrationStatus } from '../types';")
page = replace_once(page, '  getActionPillStyles,\n', '  getActionPillStyles,\n  getFilterChipProps,\n', 'filter chip helper import')
...
selection_anchor = """  const toggleSelectItem = (id: string) => {
    setSelectedIds(prev =>
      prev.includes(id) ? prev.filter(item => item !== id) : [...prev, id]
    );
  };
"""
signing_helpers = selection_anchor + """
...
"""
page = replace_once(page, selection_anchor, signing_helpers, 'signing helpers')

old_status = """              {activeTab === 'pendentes' ? (
                <>
...
new_status = """              {activeTab === 'pendentes' ? (() => {
...
page = replace_once(page, old_status, new_status, 'pending Asten status cell')

old_top = """      <div className="flex flex-wrap items-center justify-end gap-2.5 mb-3">
...
new_top = """      <div className="flex flex-wrap items-center justify-end gap-2.5 mb-3">
        {activeTab === 'pendentes' && (
          <button
            type="button"
        ...
        {activeTab === 'concluidos'"""
page = replace_once(page, old_top, new_top, 'bulk Asten action')

message_anchor = """      {downloadError && (
        <div role="alert" className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-800">
          {downloadError}
        </div>
      )}
"""
message_block = message_anchor + """      {signingMessage && (
...
"""
page = replace_once(page, message_anchor, message_block, 'Asten status messages')

# Match only the filter switcher block in the President toolbar.
filter_start = page.find('              {/* Filter Row Switcher with FILTRAR prefix following site standard */}')
if filter_start < 0:
    raise SystemExit('President filter marker not found.')
block_start = page.find('              <div className="flex items-center gap-2 shrink-0">', filter_start)
block_end_marker = '              </div>\n            </div>\n          </div>\n\n          {/* CONTEÚDO PRINCIPAL'
block_end = page.find(block_end_marker, block_start)
if block_start < 0 or block_end < 0:
    raise SystemExit('President filter block bounds not found.')
new_filters = """              <div className="flex items-center gap-2 shrink-0">
...
"""
page = page[:block_start] + new_filters + page[block_end:]
...
page = replace_once(
    ...
    'pending Asten header',
)
page = replace_once(
    page,
    "                              {columnOrder.map((colKey) => renderCell(item, colKey))}\n                            </tr>",
    "                              {columnOrder.map((colKey) => renderCell(item, colKey))}\n                              {renderAstenActionCell(proc)}\n                            </tr>",
    'pending Asten row action',
)

if "detail: 'assinaturas'" in page:
    raise SystemExit('President page still navigates to redundant Assinaturas page.')
page_path.write_text(page, encoding='utf-8')

# Remove obsolete standalone signature screen.
obsolete = Path('src/pages/SignatureQueuePage.tsx')
if obsolete.exists():
    obsolete.unlink()

# Canonical logo: remove full-canvas off-white background while preserving emblem details.
logo_path = Path('public/colenf-logo.svg')
logo = logo_path.read_text(encoding='utf-8')
outer = '<path d="M 0.00 203.50 L 0.00 0.00 L 217.50 0.00 L 435.00 0.00 L 435.00 203.50 L 435.00 407.00 L 217.50 407.00 L 0.00 407.00 L 0.00 203.50 ZM 234.00 394.54'
if outer not in logo:
    raise SystemExit('Canonical SVG background rectangle was not found.')
logo = logo.replace(outer, '<path d="M 234.00 394.54', 1)
if 'M 0.00 203.50 L 0.00 0.00 L 217.50 0.00 L 435.00 0.00' in logo:
    raise SystemExit('SVG full-canvas background still present.')
logo_path.write_text(logo, encoding='utf-8')

print('President signatures merged and canonical logo background removed.')
