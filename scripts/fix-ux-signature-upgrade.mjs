...

{
  ...
  s=s.replace("jobId:job.id,documentType:job.documentType,fileName:job.fileName.replace(/\\.pdf$/i,'__GOVBR.pdf')","jobId:`${job.id}-gov-${signer.order}`,documentType:job.documentType,fileName:job.fileName.replace(/\\.pdf$/i,`__GOVBR_${signer.order}.pdf`)");
  ...
  const replacement="{const flags=currentSettings.featureFlags||[];currentSettings.featureFlags=flags.some(flag=>flag.key==='ADMIN_TRANSFER')?flags.map(flag=>flag.key==='ADMIN_TRANSFER'?{...flag,enabled:true}:flag):[...flags,{key:'ADMIN_TRANSFER',label:'Transferência segura de administração',enabled:true}];}";
