...
const SESSION_REFRESH_AFTER_SECONDS = 15 * 60;
...
export async function attachPortalIdentity(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const identity = token ? decodeSession(token) : demoIdentity(req);
    (req as any).portalIdentity = identity;

    if (identity && !identity.isDemo) {
      const now = Math.floor(Date.now() / 1000);
      if (now - identity.issuedAt >= SESSION_REFRESH_AFTER_SECONDS) {
        const refreshed: PortalIdentity = {
          ...identity,
          issuedAt: now,
          expiresAt: now + SESSION_TTL_SECONDS
        };
        (req as any).portalIdentity = refreshed;
        writePortalSessionCookie(res, refreshed);
      }
    }
  } catch (error) {
