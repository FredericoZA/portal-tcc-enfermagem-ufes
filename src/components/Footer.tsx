...
import { MapPin, Lightbulb, Info, Car, MessageCircle, Mail, Check } from 'lucide-react';
      ...
      <div className="overflow-hidden rounded-lg border text-[11px]" style={{backgroundColor:footerBg,color:footerText,borderColor:footerBorder}}>
        <div className="grid md:grid-cols-[1fr_1.18fr] items-stretch">
          ...
          <div className="flex items-center justify-center px-3 py-2 min-h-[132px]">
            <div className="flex w-fit max-w-full items-center justify-center gap-2.5">
              <div className="min-w-0 max-w-[250px] flex flex-col justify-center items-center text-center gap-1.5">
                ...
                {whatsappUrl&&<a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1.5 font-bold uppercase tracking-wider text-[10px] rounded-lg transition-all hover:brightness-95" style={{backgroundColor:WHATSAPP_GREEN,color:whatsappText}}><WhatsAppMark className="w-3.5 h-3.5"/>WhatsApp Secretaria</a>}
                {contactEmail&&<button type="button" onClick={()=>void copyContactEmail()} className="flex items-center gap-1 rounded-md px-1.5 py-0.5 hover:bg-white/10 focus:outline-none focus:ring-1 focus:ring-white/40 transition-colors cursor-pointer" style={{color:footerMuted}} title="Clique para copiar o e-mail" aria-label={`Copiar e-mail ${contactEmail}`}>{emailCopied?<Check className="w-3 h-3"/>:<Mail className="w-3 h-3"/>}<span className="font-mono text-[9.5px]">{emailCopied?'E-mail copiado!':contactEmail}</span></button>}
              </div>
