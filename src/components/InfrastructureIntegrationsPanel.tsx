...
export const InfrastructureIntegrationsPanel: React.FC<{ isMaster: boolean }> = ({ isMaster }) => {
  ...
  const connectAsten = async () => {
    ...
    try {
      ...
      setToken('');
      setMessage({ ok: true, text: 'Asten conectada. O token foi validado e não será reexibido.' });
      await load();
  ...
  const runHomologation = async () => {
    ...
    try {
      ...
      setMessage({ ok: result.readyForProduction, text: result.readyForProduction ? 'Integrações essenciais aprovadas para produção.' : 'Há pendências de integração a resolver antes da publicação.' });
      await load();
  ...
  return <section id="infrastructure-integrations-panel" className="overflow-hidden rounded-xl border border-slate-300 bg-[#e1e6e9] p-2.5 shadow-sm space-y-2.5">
    ...
    <section className={`${card} border-emerald-200`} aria-labelledby="asten-integration-title">
      ...
      <div className="mt-2 grid gap-2 lg:grid-cols-[1fr_.85fr]">
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
          ...
          <p className="mt-2 text-[10px] leading-4 text-slate-500">Use o token, não a URL da documentação. Após conectado, o token não será mostrado novamente.</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-[10px] leading-4 text-slate-600">
          <strong className="block uppercase text-slate-700">Callback</strong>
          <p className="mt-1">Status: {status?.asten.callbackConfigured ? 'configurado' : 'pendente'} · envio: {status?.asten.dispatchEnabled ? 'habilitado' : 'bloqueado'}.</p>
          {status?.asten.callbackUrl ? <div className="mt-2 flex items-center gap-2"><code className="min-w-0 flex-1 overflow-x-auto rounded bg-white px-2 py-1">{status.asten.callbackUrl}</code><button type="button" onClick={() => void navigator.clipboard?.writeText(status.asten.callbackUrl || '')} className={action}>Copiar</button></div> : <p className="mt-2 text-amber-800">A URL de callback ainda não foi disponibilizada pelo servidor.</p>}
        </div>
      </div>
    </section>
    ...
    <div className="grid gap-2 lg:grid-cols-3">
      <section className={card} aria-label="Google Workspace e Drive">
        ...
        <button type="button" onClick={() => { window.location.href = '/api/integrations/google/oauth/start?returnTo=/?google=connected'; }} className={`${action} mt-3 w-full`}><ExternalLink className="h-3.5 w-3.5"/>Conectar Google</button>
      </section>

      <section className={card} aria-label="Supabase">
        <div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2"><Database className="h-4 w-4 text-slate-600"/><h3 className="text-sm font-black">Supabase</h3></div><State ok={supabaseReady} label={supabaseReady ? 'Pronto' : 'Pendente'}/></div>
        <p className="mt-2 text-[10px] leading-4 text-slate-600">Persistência do Portal e dados operacionais. Credenciais permanecem apenas no servidor.</p>
        <button type="button" onClick={testSupabase} disabled={working === 'supabase' || !status?.supabase.configured} className={`${action} mt-3 w-full`}>{working === 'supabase' ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <Database className="h-3.5 w-3.5"/>}Testar conexão</button>
      </section>

      <section className={card} aria-label="Vercel">
        <div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2"><Server className="h-4 w-4 text-slate-600"/><h3 className="text-sm font-black">Vercel</h3></div><State ok={vercelReady} label={vercelReady ? 'Detectado' : 'Pendente'}/></div>
        <p className="mt-2 text-[10px] leading-4 text-slate-600">Hospedagem do Portal. O projeto é detectado automaticamente pelo ambiente de execução.</p>
        <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] text-slate-600">{status?.vercel.message || 'Status ainda não carregado.'}</div>
      </section>
    </div>
