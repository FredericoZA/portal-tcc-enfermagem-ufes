...
export const InfrastructureIntegrationsPanel: React.FC<Props> = ({ isMaster }) => {
  ...
  return <div className="space-y-4">
    ...
    <section className={card} aria-label="Checklist de ativação do portal">
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{readiness.map(item=><div key={item.label} className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-[11px] font-bold ${item.ok?'border-emerald-300 bg-slate-50 text-emerald-950':'border-amber-300 bg-amber-50 text-amber-950'}`}>{item.ok?<CheckCircle2 className="h-4 w-4 shrink-0"/>:<ShieldAlert className="h-4 w-4 shrink-0"/>}{item.label}</div>)}</div>
    ...
    <div className="grid gap-3 lg:grid-cols-2">
      <section className={card}>
        <div className="flex items-center justify-between"><div className="flex items-center gap-2"><KeyRound className="h-5 w-5 text-orange-600"/><h3 className="font-black">Asten Assinatura · opcional</h3></div><State ok={Boolean(status?.asten.configured&&status?.asten.callbackConfigured)} label={status?.asten.configured ? (status?.asten.callbackConfigured?'Conectada':'Callback pendente') : 'Pendente'}/></div>
