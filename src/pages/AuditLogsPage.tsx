...
import { Download, FileCheck2, RefreshCw, RotateCcw, Upload } from 'lucide-react';
...
export const AuditLogsPage: React.FC = () => {
  ...
  const load = async (silent = false) => {
    ...
    try {
      ...
      portalNotice('Não foi possível carregar o registro de logs.');
    } finally {
      if (silent) window.setTimeout(() => setRefreshing(false), 300);
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter((log) => {
      ...
      if (!term) return true;
      return [log.actorEmail, log.action, log.processId, log.entityType, log.entityId]
  ...
  const downloadBackup = async () => {
    try {
      ...
      anchor.href = url;
      anchor.download = `backup-portal-tcc-${new Date().toISOString().slice(0, 10)}.json`;
      anchor.click();
      URL.revokeObjectURL(url);
    } catch (error: any) {
      portalNotice(`Erro ao gerar backup: ${error?.message || 'falha inesperada'}`);
    }
  };
  ...
  const restoreBackup = (file?: File) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        await apiClient.restoreFullBackup({ ...parsed, confirmHash: preview.confirmHash });
        window.location.reload();
      } catch (error: any) {
        portalNotice(`Erro ao restaurar backup: ${error?.message || 'falha inesperada'}`);
      } finally {
  ...
  const rollback = async (log: AuditLog) => {
    ...
    try {
      await apiClient.rollbackAuditLog(log.id);
      await load(true);
    } catch (error: any) {
      portalNotice(`Erro ao restaurar registro: ${error?.message || 'falha inesperada'}`);
    } finally {
      setRollingBack('');
    }
  };
  ...
  if (!isMasterAdmin) {
    return <section className="rounded-xl border border-slate-300 bg-white p-6 text-sm font-semibold text-slate-700">Registro de logs disponível somente para o usuário Master.</section>;
  }
  ...
  return <div id="audit-logs-page" className="space-y-0 overflow-hidden rounded-2xl border border-slate-300 bg-[#d5dce0] shadow-sm">
    <header className="flex flex-wrap items-center justify-between gap-3 bg-[#005830] px-4 py-3 text-white">
      <div className="min-w-0">
        ...
        <p className="mt-0.5 text-[10px] font-semibold text-white/75">Auditoria, restauração e rastreabilidade do Portal · {filtered.length} registro(s)</p>
      </div>
    ...
    <div className="border-t-2 border-white bg-[#337959] px-4 py-2 text-[10px] font-bold text-white/90">
      {search || startDate || endDate ? `Filtros ativos · exibindo ${shown.length} de ${filtered.length} resultado(s)` : `Histórico completo · exibindo ${shown.length} de ${logs.length} registro(s)`}
    </div>

    <div className="p-3 sm:p-4">
      {loading ? <div className="rounded-xl border border-slate-300 bg-white p-8 text-center text-xs font-semibold text-slate-500">Carregando histórico…</div> : shown.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center text-xs font-semibold text-slate-500">Nenhum registro encontrado com os filtros atuais.</div> : <div className="overflow-x-auto rounded-xl border border-slate-300 bg-white">
        <table className="w-full min-w-[860px] border-collapse text-left text-xs">
          <thead><tr className="border-b border-slate-300 bg-[#e1e6e9] text-[10px] font-black uppercase tracking-wide text-slate-800">{activeColumns.map((key) => <th key={key} className={`px-3 py-2.5 ${key === 'options' ? 'text-right' : ''}`}>{LOG_COLUMNS.find((column) => column.key === key)?.label || key}</th>)}</tr></thead>
