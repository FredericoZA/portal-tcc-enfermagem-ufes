...
const tools = [
  ...
  },
  {
    ...
    text: 'Nome do curso, domínio, e-mails, logotipos e elementos visuais são configuráveis. Assim, a mesma base pode receber a identidade de outra unidade sem copiar a operação da Enfermagem.',
  },
] as const;
...
const models = [
  {
    slug: 'convite',
    label: 'Convite de Defesa',
    description: 'Base utilizada para preparar o convite institucional da banca e da apresentação.',
  },
  {
    slug: 'ata',
    label: 'Ata de Defesa',
    description: 'Modelo da Ata utilizada após a defesa para registrar o resultado e o parecer.',
  },
  {
    slug: 'termo',
    label: 'Termo de Autorização',
    description: 'Modelo utilizado na etapa de autorização e publicação do trabalho acadêmico.',
  },
  {
    slug: 'declaracao',
    label: 'Declaração da Banca',
    description: 'Modelo da declaração de participação dos membros da banca examinadora.',
  },
] as const;
...
export const PortalReplicationPage: React.FC = () => (
  <div id="portal-replication-page" className="mx-auto max-w-5xl space-y-3 py-2">
    ...
    <section className="grid gap-3 lg:grid-cols-[0.82fr_1.45fr]">
      <article className="rounded-2xl border border-slate-300 bg-[#f0f0f0] p-3 shadow-sm sm:p-4">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-3 inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-black text-slate-900 shadow-sm hover:brightness-95"
        >
      ...
      <article className="rounded-2xl border border-slate-300 bg-[#f0f0f0] p-3 shadow-sm sm:p-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {models.map(({ slug, label, description }) => (
            <div key={slug} className="flex min-h-28 flex-col justify-between rounded-xl border border-slate-200 bg-white p-3 shadow-2xs">
              <div>
              ...
              <a
                ...
                className="mt-3 inline-flex w-fit items-center gap-1.5 rounded-lg border border-slate-300 bg-[#f0f0f0] px-2.5 py-1.5 text-[10px] font-black uppercase tracking-wide text-slate-800 shadow-sm hover:brightness-95"
              >
